...
class PointLabeler:
    ...
    def __init__(self, labeler_config):
        ...
        self.anno_frame_ids = list(
            range(labeler_config.ANNOTATE.FRAME_IDS[0], labeler_config.ANNOTATE.FRAME_IDS[1],
        ...
        self.vis_frame_ids = list(
            range(labeler_config.VISUALIZE.FRAME_IDS[0], labeler_config.VISUALIZE.FRAME_IDS[1],
    ...
    def visualize(self, vis_meshes=False):
        ...
        for frame_id in tqdm(self.vis_frame_ids, desc="Reading the point clouds"):
            if not os.path.exists(
                    os.path.join(data_dir, "point_clouds",
                                 f"{str(frame_id).zfill(4)}_pointcloud.ply")):
                continue

            point_cloud = o3d.io.read_point_cloud(
                os.path.join(data_dir, "point_clouds", f"{str(frame_id).zfill(4)}_pointcloud.ply"))
            labels = labelutil.read_labels(
                os.path.join(data_dir, "labels", f"{str(frame_id).zfill(4)}_pointcloud.label"))
            ...
            if vis_meshes:
                meshes = load_mesh(
                    os.path.join(self.mesh_data_dir, self.vis_trial, PHASE, "smpl_files"), frame_id)

                for i, mesh in enumerate(meshes):
                    mesh.paint_uniform_color([0.7, 1, 1])
                    vis.add_geometry(f"mesh_fr_{frame_id}_{i}", mesh)
    ...
    def init_output(self):
        ...
        for trial in self.anno_trials:
            ...
            if not os.path.exists(data_dir):
                logger.warn(
                    f"There is no data for the trial {trial} in {data_dir}.\nShutting down now...")
                exit(0)
            ...
            for phase in os.listdir(data_dir):
                ...
                if len(glob(trial_label_path + '/point_clouds/*.ply')) != len(
                        glob(phase_data_dir + '/cn01/*.ply')):
                    # We have to create the point clouds and labels ourselves
                    logger.info(
                        f"There is no or insufficient data in {trial_label_path}. Writing data now..."
                    )
                    # Writing all the data ourselves
                    point_cloud_output_dir = os.path.join('data', 'label_data', trial, phase,
                                                          'point_clouds')
                    ...
                    for frame_id in tqdm(_anno_frame_ids, desc="Writing merged point clouds"):
                        ...
                        if not os.path.exists(
                                os.path.join(phase_data_dir, cameras[0],
                                             f"{file_id}_pointcloud.ply")):
                            continue
                        ...
                        o3d.io.write_point_cloud(
                            pcd_file := os.path.join(point_cloud_output_dir,
                                                     f"{file_id}_pointcloud.ply"),
                            merged_point_cloud)
                        ...
                        with open(
                                ...
                                os.path.join(labels_dir,
                                             f"{str(frame_id).zfill(4)}_pointcloud.label"),
                                "bw") as f:
                            f.write(contents)
    ...
    def calibrate(self):
        with open('calibrations.json', 'r') as f:
            ...
            for cal_id in self.cal_trials:
                ...
                calibration_dict = cal_main(self.x, self.y, self.z, self.ground, first_pcd,
                                            self.voxel_space, self.nb_points, self.radius)
...
def create_first_pcd(trials):
    ...
    for trial in trials:
        ...
        if os.path.exists(path_to_phase := os.path.join("data", "pointcloud_export_demo", trial, phase)):
            path = path_to_phase
